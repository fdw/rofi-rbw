...
| `--delay`              |              | float in seconds (default `1.0`)                              | Duration to wait when a `delay` step is encountered in a keybinding sequence (e.g. `Alt+c:copy:username:delay:password`), for both `type` and `copy` actions.                                                                                                               |
